    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        if (LastActiveLink)
        {
            ...
            output.Attributes.SetAttribute("class",
                "relative rtl:mr-5 ltr:ml-5 rtl:pr-0.5 ltr:pl-0.5 before:content-[''] before:w-[14px] before:h-[14px] before:bg-chevron-white before:transform rtl:before:rotate-[90deg] ltr:before:rotate-[-90deg] before:bg-[length:100%] rtl:before:-mr-[1.125rem] ltr:before:-ml-[1.125rem] before:absolute before:my-auto before:inset-y-0 dark:before:bg-chevron-white text-white/70");
            output.Content.SetHtmlContent($"<a href=\"javascript:void(0)\">{cc.GetContent()}</a>");
            return;
        }
